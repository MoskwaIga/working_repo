def plot_distribution_split(
    ...
    split_df = df[df[month_col] == split_month]
    after_df = df[df[month_col] > split_month]
    ...
    if segment_col is None:
        ...
        for ax, feature in zip(axes_flat, features):
            ...
            arrays = [
                arr
                for arr in [before, split_vals, after]
                if len(arr) > 0
            ]
            ...
            if len(split_vals) > 0:
                sns.histplot(
                    ...
                    alpha=0.35,
                    color="#F5A623",
                    label=f"{split_month} (n={len(split_vals):,})",
                    line_kws={"linewidth": 1.8},
                )
            ...
            if len(after) > 0:
                sns.histplot(
                    ...
                    alpha=0.35,
                    color="#E24B4A",
                    label=f"po {split_month} (n={len(after):,})",
                    line_kws={"linewidth": 1.8},
                )
        ...
        fig.suptitle(
            f"Rozkład przed / {split_month} / po {split_month}",
            fontsize=12,
            fontweight="bold",
        )
    ...
    for row_idx, feature in enumerate(features):
        ...
        for _, segment_mask in segments:
            ...
            for arr in [before_seg, split_seg, after_seg]:
                if len(arr):
    ...
    fig.suptitle(
        f"Rozkład przed / {split_month} / po {split_month} | segmentacja: {segment_col}",
        fontsize=12,
        fontweight="bold",
    )
