...
def psi_month_vs_all(
    ...
    """
    Reference dla okna W = populacja bez okna W
    → breakpointy z populacji bez bieżącego okna. ✅
    """
    ...
    for w, w_months in win_map.items():
        mask_cur  = df[month_col].isin(w_months)
        reference = df[~mask_cur]
        current   = df[mask_cur]

        r = compare(reference, current, feature_cols, bins, label="month_vs_all")
        r["window"] = _window_label(w_months, window_size)
        results.append(r)
